...
export default function Quiz() {
  ...
  if (key === 'intake') {
    const emailValid = EMAIL_RE.test(intake.email.trim());
    const showEmailError = emailTouched && intake.email.length > 0 && !emailValid;
    return (
      <div style={wrap}>
        ...
        {['name', 'business', 'email'].map((f) => (
          <div key={f}>
            <input
              ...
              onBlur={() => f === 'email' && setEmailTouched(true)}
              style={{
                ...inputStyle,
                borderColor: f === 'email' && showEmailError ? 'var(--coral-deep)' : 'var(--border)',
              }}
            />
            {f === 'email' && showEmailError && (
              <div style={{ fontSize: 12, color: 'var(--coral-deep)', marginTop: -6, marginBottom: 12 }}>
                That doesn't look like a valid email — double check it so your report isn't lost.
              </div>
            )}
          </div>
        ))}
        <button className="btn-primary" disabled={!intake.name || !emailValid} onClick={advance} style={{ marginTop: 8 }}>
          Continue
        </button>
      </div>
    );
