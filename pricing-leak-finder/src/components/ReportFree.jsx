...
export default function ReportFree({ intake, ranking, confidence, priceLabel, sub, onUnlock }) {
  ...
  const rawPrice = Number(priceLabel.replace(/[^0-9.]/g, '')) || 3800;
  // Never let the thing already flagged as a leak also get praised as
  // "already working well" — exclude Primary and Secondary before picking
  // the best-scoring one from what's left.
  const leakyKeys = new Set([
    ...(Array.isArray(ranking.primary) ? ranking.primary : [ranking.primary]),
    ranking.secondary,
  ]);
  const healthiest = [...ranking.entries]
    .filter(([k]) => !leakyKeys.has(k))
  ...
  return (
    <>
      ...
      <Card style={{ marginBottom: 16, background: 'var(--mint-light)', border: 'none' }}>
        ...
        <CapText>
          <b>{DIMENSIONS[healthiest].name}</b> — {DIMENSIONS[healthiest].whatItIs} — is the one part of your pricing conversations that's already working well. Worth knowing, so you don't spend time fixing something that isn't actually broken.
        </CapText>
      </Card>

      <Card style={{ marginBottom: 16 }}>
        <SectionTag>Your Risk If Unaddressed</SectionTag>
        <div style={{ marginBottom: 14 }}><RiskDiagram price={rawPrice} confidencePct={confidence.conf} /></div>
