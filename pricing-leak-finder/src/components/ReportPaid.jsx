...
export default function ReportPaid({ intake, ranking, archetype, priceLabel, sub }) {
  ...
  return (
    <>
      ...
      {OBJECTIONS.filter((o) => o.featured).map((o, i) => (
        <Card key={`f-${i}`} style={{ marginBottom: 12, background: 'var(--cream-2)', border: 'none' }}>
          <div style={{
      ...
      <Card style={{ marginBottom: 20 }}>
        {OBJECTIONS.filter((o) => !o.featured).map((o, i) => (
          <div key={i} style={{ padding: '14px 0', borderTop: i > 0 ? '1px solid var(--border)' : 'none' }}>
            <div style={{ fontFamily: 'var(--serif)', fontStyle: 'italic', fontSize: 13, marginBottom: 6 }}>{sub(o.q)}</div>
            <div style={{ fontSize: 12.5, color: 'var(--espresso)', fontWeight: 500, marginBottom: 6 }}>{sub(o.a)}</div>
            <div style={{ fontSize: 12, color: 'var(--espresso-70)', lineHeight: 1.5 }}>{sub(o.why)}</div>
          </div>
        ))}
      </Card>
