...
import {
  DIMENSIONS, COMPOUNDS, ARCHETYPES, OBJECTIONS, TRADE_MENU,
...
export default function ReportPaid({ intake, ranking, archetype, priceLabel, sub }) {
  ...
  return (
    <>
      ...
      {Object.entries(primaryContent.archetypeTemplates || {}).map(([key, line]) => {
        ...
        return (
          <Card key={key} style={{ marginBottom: 12 }}>
            ...
            <div style={{ background: 'var(--mint-light)', borderRadius: 10, padding: '10px 14px', fontSize: 13 }}>{sub(line)}</div>
          </Card>
        );
      })}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10 }}>
          {TRADE_MENU.map((t, i) => (
            <div key={i} style={{ background: 'var(--mint-light)', borderRadius: 12, padding: '12px 14px' }}>
              ...
              <div style={{ fontFamily: 'var(--serif)', fontStyle: 'italic', fontSize: 11.5, color: 'var(--espresso-70)', lineHeight: 1.5 }}>{sub(t.line)}</div>
            </div>
          ))}
        </div>
      </Card>
